...
function money(v) {
  ...
  // text like "от 450" / "450+" / "≈450"
  const m = raw.match(/(от\s*)?(\d+)/i);
  if (m) {
    const prefix = m[1] ? "от " : "";
    return `${prefix}${m[2]} ₽`;
  }
  ...
  return blocks
    .map((b) => {
      ...
      if (isTitleLine(first)) {
        const titleHtml = `<div class="dTitle">${escapeHTML(first)}</div>`;
...
function pickFirstField(obj, keys) {(obj, keys) {
  for (const k of keys) {
    const v = obj?.[k];
...
Если вы уже покупали товары из текущего ассортимента — для вас будут подготовлены новые уникальные наборы, которые позже появятся в ассортименте магазина.",
      ].join("
