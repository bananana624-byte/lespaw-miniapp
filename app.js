...
const POSTER_PACKS = [
  ["p10x15_8", "10 фотопостеров 10 × 15 см", 250],
  ["p21x30_5", "5 фотопостеров 21 × 30 см", 450],
  ["p_mix", "10 фотопостеров 10 × 15 см + 5 фотопостеров 21 × 30 см", 700],
];
const POSTER_PAPERS = [
...
const LAMINATION_EXAMPLES = [
  // ===== ПЛЁНКА (основа) =====
  {
    ...
    subtitle: "С бликами",
    description:
      "Плёнка с гладкой глянцевой поверхностью, которая усиливает яркость цветов и делает изображение более контрастным." +
      "Хорошо подчёркивает детали и насыщенность иллюстраций." +
      "Свет отражается от поверхности, создавая характерный блеск и эффект «свежей печати».",
...
function applySurpriseInsideOverride(rawDesc, p) {
  ...
  const replacementLines = isEnvelope
    ? [
        "📦 Внутри",
        "• 1 набор наклеек",
        "• 10 глянцевых фотопостера 10 × 15 см",
    ...
    : [
        ...
        "• 1 набор значков",
        "• 1 набор наклеек",
        "• 15 глянцевых фотопостера 10 × 15 см",
        "• 5 глянцевых фотопостера 21 × 30 см",
        "• 4 3D-стикера (3 × 3 см)",
        "• 1 круглый брелок (44 мм)",
      ];
...
function defaultShortByType(p) {
  ...
  if (typeKey === "sticker") return "Лист наклеек • 16×25 см";
...
function defaultFullByType(p) {
  ...
  if (rawType === "sticker_set") {
    return [
      "✨ О товаре\nЯркие наклейки с чёткой печатью. Подойдут для декора ноутбуков, планшетов, ежедневников и других гладких поверхностей.",
  ...
  if (groupKey === "pin_single" || groupKey === "pin_set" || typeKey === "pin") {
    if (groupKey === "pin_single" || isPinSingleType(p?.product_type)) {
      return [
        "✨ О товаре\nОдин круглый значок — для тех, кто хочет конкретный дизайн без набора. Удобно комбинировать и собирать свою коллекцию, или взять один любимый.",
        "📏 Характеристики\n• Размер значка: круглый - 44 мм\n• Материал: металл\n• Крепление: булавка сзади",
      ].join("\n\n");
    }

    return [
      "✨ О товаре\nНабор из 6 круглых значков.\nХорошо подойдут для рюкзаков, сумок, курток или коллекций — лёгкие, удобные и приятные в использовании.",
      "📦 В наборе\n• 6 значков",
  ...
  if (baseKey === "sticker") {
    return [
      "✨ О товаре\nЯркие наклейки. Подойдут для декора ноутбуков, планшетов, ежедневников и других гладких поверхностей.",
  ...
  if (baseKey === "poster") {
    return [
      ...
      "📦 Варианты наборов\n• 10 фотопостеров 10 × 15 см — 250 ₽\n• 5 фотопостеров 21 × 30 см — 450 ₽\n• 10 фотопостеров 10 × 15 см + 5 фотопостеров 21 × 30 см — 700 ₽",
      "🖨️ Бумага для печати\n• Глянцевая — выразительный блеск\n• Матовая — мягкая цветопередача без бликов\n• Сатиновая — лёгкий шелковистый блеск",
